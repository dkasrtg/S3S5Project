...
public class FabricationMeubleServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<VMouvementMeuble> vMouvementMeubles = VMouvementMeuble.selectByTypeMouvement(connection, MouvementMeuble.ENTREE, dateDebut, dateFin);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Integer idFormuleMeuble = FormuleMeuble.existByIdMeubleAndTailleMeuble(connection, idMeuble, idTailleMeuble);
            if (idFormuleMeuble==-1) {
                throw new FormuleMeubleTailleNotExistException();
            }
            double prixTotal = 0;
            List<VDetailFormuleMeuble> vDetailFormuleMeubles = VDetailFormuleMeuble.selectByIdFormuleMeuble(connection, idFormuleMeuble);
            MouvementMeuble mouvementMeuble = new MouvementMeuble(null, dateFabrication, idFormuleMeuble, quantite, 0.0, 0.0, MouvementMeuble.ENTREE, -1);
            ...
            for (int i = 0; i < vDetailFormuleMeubles.size(); i++) {
                ...
                List<VMateriauRestant> vMateriauRestants = VMateriauRestant.selectByIdMateriauWhereDateMouvementBefore(connection, vDetailFormuleMeubles.get(i).getIdMateriau(), dateFabrication);
                ...
                if (quantitemateriau!=0) {
                    connection.rollback();
                    throw new QuantiteInsufficientForMeubleException(vDetailFormuleMeubles.get(i).getNomMateriau(), quantitemateriau);
                }
                prixTotal += prixMateriau;
            }
